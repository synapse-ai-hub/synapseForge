"""Runner del agente con tools para la creación de tools externas.

Implementa un loop de ejecución similar a ``loop.py`` pero sin SSE:
- El agente tiene acceso a tools (read, write, edit, shell, list_dir).
...
async def run_tool_agent(
    ...
    for iteration in range(_MAX_ITERATIONS):
        ...
        for tc in tool_calls:
            ...
            logger.info(
                "Tool %s: %.2fs, %d chars",
                tc_name, elapsed, len(str(result_content))
            )
